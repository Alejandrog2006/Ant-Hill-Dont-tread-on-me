...
Status game_actions_use(Game *game)
{
    ...
    {
        Id obj_id = set_get_id_at(inventory_get_objects(player_get_inventory(player)), i);
        object = game_get_object_by_id(game, obj_id);
    ...
    if (chr_name && chr_name[0] != '\0')
    {
        ...
        if (!target_character || character_get_following(target_character) != player_get_id(player))
        {
            game_set_last_message(game, " That character is not following you.");
            return ERROR;
        }

        character_set_health(target_character, character_get_health(target_character) + object_get_health(obj));
    ...
    player_del_object(player, object_get_id(obj));
    game_set_object_location(game, NO_ID, object_get_id(obj));
