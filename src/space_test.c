...
/** 
 * @brief Test for setting the name of a space.
 */
...
int main(int argc, char** argv) {
    ...
    if (all || test == 15) test1_space_get_object_locations();
    if (all || test == 16) test2_space_get_object_locations();
    if (all || test == 17) test1_space_print();
    if (all || test == 18) test2_space_print();
...
void test1_space_get_object_locations() {
    ...
    PRINT_TEST_RESULT(space_get_object_locations(s) != NULL);
    space_destroy(s);
}
