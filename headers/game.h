 ...
 * @return 
 *  The links array if everything went correctly
 ...
 * @param game A pointer to the game struct
 * @return
 *  A pointer to the n_links variable if everything went correctly
 *  NULL if something went wrong
 */
 ...
 * @return 
 *  The destination id of the link that begins in id_orig with the direction dir
 *  NO_ID if something went wrong or there's no such link
 */
 ...
 * @param dir The direction of that link
 * @return
 *  TRUE if the link between those two spaces is open
 *  FALSE if the link between those two spaces is closed or something went wrong
 */
